...
export const Navbar = () => {
    ...
    const onLogout = () => {
        navigate('/login', {
            replace: true //evita que la persona pueda regresar al historial anterior
        });
    }
        ...
        <nav className="navbar navbar-expand-sm navbar-dark bg-dark">
            ...
            <div className="navbar-collapse collapse w-100 order-3 dual-collapse2 d-flex justify-content-end p-2">
                <ul className="navbar-nav ml-auto">
                    ...
                    {/* <NavLink className={ ({ isActive }) => `nav-item nav-link ${ isActive ? 'active' : '' }`} to="/login">Logout</NavLink> */}
                </ul>
            </div>
        </nav>
    )
